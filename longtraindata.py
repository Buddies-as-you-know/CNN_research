...
def labeler():
  ...
  with open("C:\\Users\\owner\\Desktop\\EEGdata\\datasample\\commandlongtrain.txt", "r") as tf:
    lines = tf.read().split(' ')
  #print(lines)
...
def dataset(url):
  ...
  ts = d2[125*10:125*40*7*4+125*10,:]
  return ts
...
if __name__ == "__main__":
    data1 = dataset("C:\\Users\\owner\\Desktop\\EEGdata\\datasample\\long.txt")
    ...
    label = labeler()
    print(len(label))
    test_label =np.array(label).reshape((-1, 1))
    """"
    ...
    """
    np.savetxt('train_data.txt',data1)
    np.savetxt('train_label.txt',test_label)